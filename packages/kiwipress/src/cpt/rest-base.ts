...
function invalidRestBase(restBase: string): Error {
    return new Error(
        `Invalid WordPress restBase "${restBase}". Use a single collection slug such as "books" or "product".`
    );
}
 ...
 */
export function sanitizeRestBase(restBase: string): string {
