...
export function isStackShaped(value: unknown): value is StackConfig {
...
export function servicesNeedsLegacyWarning(config: GrapeConfig): boolean {
    return Boolean(config.services && Object.keys(config.services).length > 0 && !isStackShaped(config.services));
}

function readRequired(baseDir: string, rel: string, label: string): string {
...
function destOnDroplet(workdir: string, dest: string): string {
    if (dest.startsWith("/")) {
        return dest;
    }
...
function composeDest(workdir: string, sourcePath: string, index: number, total: number): string {
...
function renderEnvFile(keys: Record<string, string>): string {
...
export function generateDefaultBootstrap(stack: ResolvedStack): string {
...
export function resolveStack(stack: StackConfig, options: ResolveStackOptions = {}): ResolvedStack {
    const baseDir = options.baseDir ?? process.cwd();
    const workdir = stack.workdir ?? DEFAULT_STACK_WORKDIR;
    const name = stack.name ?? `stack-${stack.droplet}`;
    const files: StackFile[] = [];
    const composeDests: string[] = [];

    const composeSources: string[] = [];
...
export function resolveDeclaredStacks(
    config: GrapeConfig,
...
export function generateStackUserData(stack: ResolvedStack): string {
    const doc = {
        package_update: true,
...
function userDataContentType(body: string): string {
    return body.trimStart().startsWith("#cloud-config")
        ? 'text/cloud-config; charset="utf-8"'
        : 'text/x-shellscript; charset="utf-8"';
}

export function mergeUserData(existing: string | undefined, generated: string): string {
    if (!existing?.trim()) {
        return generated;
    }
    return [
...
export function stackPlanDetails(stack: ResolvedStack): Record<string, string> {
    return {
        droplet: stack.droplet,
