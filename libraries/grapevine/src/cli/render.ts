...
import type { GrapePlan, PlannedResource } from "../config/plan.js";
...
function countLine(counts: ResourceCounts): string {
    const parts = RESOURCE_KINDS.filter((kind) => counts[kind] > 0).map(
        (kind) => `${counts[kind]} ${kind}`
    );
    return parts.length > 0 ? parts.join(", ") : "none";
}

function formatDetail(resource: PlannedResource): string {
...
export function renderValidate(plan: GrapePlan, source: string, heading = "Valid grape config"): void {
    println(heading);
    println();
    println(
        formatLabeled([
...
export function destroySummary(targets: DestroyTarget[]): string {
    if (targets.length === 0) {
        return "No matching DigitalOcean resources to destroy.";
    }
    const lines = targets.map(
        (target) => `  ${target.kind.padEnd(14)}  ${target.name}${target.id !== undefined ? `  id=${target.id}` : ""}`
    );
...
export function overlapWithConfig(plan: GrapePlan, inventory: LiveInventory): StatusOverlap[] {
    const rows: StatusOverlap[] = [];
...
export function renderLiveStatus(inventory: LiveInventory): void {
    ...
    if (inventory.domains.length === 0) {
        ...
        println(
            formatTable(
                inventory.domains.map((domain) => ({ name: domain.name })),
                [{ key: "name", header: "NAME" }]
            )
        );
    }
...
export function renderOverlap(rows: StatusOverlap[]): void {
