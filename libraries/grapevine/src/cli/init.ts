...
export function resolveBlueprintsDir(from = fileURLToPath(import.meta.url)): string {
    ...
    throw new CliError(
        "Could not find packaged blueprints. Reinstall @citrusworx/grapevine or run grape from the grapevine package."
    );
}
...
export function resolveBlueprint(
    ...
    const matches = blueprints.filter((item) => {
        const names = [item.id, item.file.replace(/\.ya?ml$/i, ""), ...item.aliases].map((name) =>
            name.toLowerCase()
        );
    ...
    if (matches.length > 1) {
        throw new CliError(
            `Ambiguous blueprint "${query}". Matches: ${matches.map((item) => item.id).join(", ")}`
        );
    }
    throw new CliError(
        `Unknown blueprint "${query}". Run grape init --list to see available starters.`
    );
}
