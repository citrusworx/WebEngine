...
function mergedOptions(command: Command, extra: Partial<CommandOptions> = {}): CommandOptions {
    const opts = command.optsWithGlobals() as CommandOptions;
    return { ...opts, ...extra };
}
...
export function createProgram(): Command {
    ...
    program
        .name("grape")
        .description(
            "Grapevine DigitalOcean CLI. KiwiEngine is not required; kiwi --grape delegates to this binary when it is on PATH."
        )
        .version(pkg.version)
        .showHelpAfterError()
    ...
    addCommonOptions(
        program
            .command("validate")
            .description("Schema-validate a grape config without calling DigitalOcean")
    ...
    addCommonOptions(
        program
            .command("plan")
            .description("Print the resource graph that apply would create (no DigitalOcean mutations)")
    ).action(async (_opts, command: Command) => {
    ...
    addCommonOptions(
        program
            .command("apply")
            .description("Create the resources declared in a grape config")
    ).action(async (_opts, command: Command) => {
    ...
    addCommonOptions(
        program
            .command("status")
            .description("Show live DigitalOcean resources, and optionally compare them to a config")
    ).action(async (_opts, command: Command) => {
    ...
    addCommonOptions(
        program
            ...
            .option("-f, --force", "Overwrite grape.config.yaml if it exists")
    ).action(async (blueprint: string | undefined, _opts, command: Command) => {
