...
import { createApp, listApps, waitForAppDeployment } from "../providers/digitalocean/apps/apps.js";
import { cdnEndpointIsLive, createCdnEndpoint, listCdnEndpoints, resolveCdnOrigin, updateCdnEndpoint, waitForCdnEndpoint } from "../providers/digitalocean/cdn/cdn.js";
...
import { createDatabase, getDatabase, listDatabases, pickDatabaseConnection, waitForDatabase } from "../providers/digitalocean/databases/databases.js";
import { createDroplet, listAllDroplets } from "../providers/digitalocean/droplet/droplet.js";
import { createFireWall, listAllFirewalls, updateFirewall } from "../providers/digitalocean/firewall/firewall.js";
import { createAlertPolicy, listAlertPolicies } from "../providers/digitalocean/monitoring/monitoring.js";
import { createDomain, createDomainRecord, listAllDomainRecords, listAllDomains } from "../providers/digitalocean/networking/domains.js";
...
import { lookupByName, lookupCdnByOrigin, lookupSSHKey, lookupVPC, lookupWhere } from "./adopt.js";
...
import { publishStaticSites } from "./static-publish.js";
...
export async function applyGrapeConfig(config, options = {}) {
    ...
    for (const key of resources.ssh_keys ?? []) {
        ...
        if (foundKey.status === "unique" && foundKey.resource) {
            ...
            record(result, { kind: "ssh_key", name: adopted.name, action: "adopted", id: adopted.id }, `Adopting existing SSH key "${adopted.name}" (id ${adopted.id})`);
            continue;
        }
        if (!publicKey && key.generate) {
    ...
    for (const vpc of resources.vpcs ?? []) {
        ...
        if (found.status === "unique" && found.resource) {
            ...
            record(result, { kind: "vpc", name: adopted.name, action: "adopted", id: adopted.id }, `Adopting existing VPC "${adopted.name}" (id ${adopted.id})`);
            continue;
        }
    ...
    for (const database of resources.databases ?? []) {
        ...
        if (found.status === "unique" && found.resource) {
            ...
            record(result, { kind: "database", name: current.name, action: "adopted", id: current.id }, `Adopting existing database "${current.name}" (id ${current.id}); engine, size, and region are not changed`);
        }
        else {
    ...
    for (const entry of resources.droplets ?? []) {
        ...
        if (foundDroplet.status === "unique" && foundDroplet.resource) {
            ...
            record(result, { kind: "droplet", name: existing.name, action: "adopted", id: existing.id }, `Adopting existing droplet "${existing.name}" (id ${existing.id}); skipping create`);
            if (resolvedStack) {
                record(result, {
                    kind: "stack",
                    name: resolvedStack.name,
    ...
    const declaredDropletNames = new Set((resources.droplets ?? []).map((entry) => unwrapDropletEntry(entry).name));
    ...
    for (const firewall of resources.firewalls ?? []) {
        ...
        if (foundFirewall.status === "unique" && foundFirewall.resource) {
            ...
            if (updated.changed) {
                ...
                record(result, { kind: "firewall", name: existing.name, action: "adopted", id: existing.id }, `Adopting existing firewall "${existing.name}" (id ${existing.id}); skipping create`);
            }
            continue;
        }
        const payload = {
    ...
    for (const domain of resources.domains ?? []) {
        ...
        let domainAction = "created";
        if (found.status === "unique") {
            domainAction = "adopted";
            record(result, { kind: "domain", name: domain.name, action: "adopted" }, `Adopting existing domain "${domain.name}"; the apex address is not changed`);
        }
        else {
    ...
    for (const lb of resources.load_balancers ?? []) {
        ...
        if (found.status === "unique" && found.resource?.id) {
            ...
            record(result, { kind: "load_balancer", name: lb.name, action: "adopted", id: found.resource.id }, `Adopting existing load balancer "${lb.name}" (id ${found.resource.id}); forwarding rules are not changed`);
            continue;
        }
    ...
    for (const policy of resources.alert_policies ?? []) {
        const found = lookupWhere(liveAlerts, (item) => item.description === policy.description, (item) => item.uuid);
    ...
    for (const app of resources.apps ?? []) {
        const found = lookupWhere(liveApps, (item) => item.spec?.name === app.spec.name, (item) => item.id);
        if (skipLookup(result, "app", app.spec.name, found, "app")) {
            continue;
        }
        if (found.status === "unique" && found.resource) {
            const ready = app.wait === true
            ...
            record(result, { kind: "app", name: ready.spec.name, action: "adopted", id: ready.id }, `Adopting existing app "${ready.spec.name}" (id ${ready.id}); the spec is not updated`);
            continue;
        }
    ...
    if ((resources.spaces ?? []).length > 0) {
        ...
        for (const space of resources.spaces ?? []) {
            ...
            if (found.status === "unique" && found.resource) {
                ...
                record(result, { kind: "space", name: found.resource.name, action: "adopted" }, `Adopting existing Space "${found.resource.name}"; region and ACL are not changed`);
                continue;
            }
    ...
    for (const certificate of resources.certificates ?? []) {
        ...
        if (found.status === "unique" && found.resource) {
            current = found.resource;
            record(result, { kind: "certificate", name: current.name, action: "adopted", id: current.id }, `Adopting existing certificate "${current.name}" (id ${current.id})`);
        }
        else {
        ...
        if (current.state !== "verified") {
            if (certificate.wait === false) {
                warnings.push(`Certificate "${current.name}" is ${current.state ?? "not verified"}; wait is false, so apply did not poll it`);
            }
            else {
        ...
        for (const endpoint of resources.cdn ?? []) {
            ...
            if (foundCdn.status === "unique" && foundCdn.resource) {
                ...
                if (ttlChanges) {
                    ...
                    record(result, { kind: "cdn", name: origin, action: "adopted", id: current.id }, `Adopting existing CDN endpoint for origin "${origin}" (id ${current.id}); custom domain and certificate are not changed`);
                }
            ...
            if (endpoint.certificate && !certificateId) {
                ...
                if (match.status === "ambiguous") {
                    ...
                    warnings.push(`Skipping CDN create for origin "${origin}" because certificate "${endpoint.certificate}" is ambiguous`);
                    continue;
                }
            ...
            if (certificateId && certificateState !== "verified") {
                ...
                const known = result.certificates.find((item) => item.id === certificateId || item.name === endpoint.certificate);
                if (known) {
                    known.state = ready.state;
                }
            }
            const created = await ensureCdnUsable(await createCdnEndpoint({
                origin,
                ttl: endpoint.ttl,
                certificate_id: certificateId,
                custom_domain: endpoint.custom_domain
            }), endpoint.wait, endpoint.wait_seconds, warnings, origin);
    ...
    if (staticSites.length > 0) {
        const published = await publishStaticSites(staticSites, {
            baseDir,
...
function waitMs(seconds, fallbackMs) {
 ...
 */
async function ensureCdnUsable(endpoint, wait, waitSeconds, warnings, origin) {
    if (cdnEndpointIsLive(endpoint)) {
        return endpoint;
    }
    if (wait === false) {
        warnings.push(`CDN endpoint for origin "${origin}" has no hostname yet; wait is false, so apply did not poll it`);
        return endpoint;
    }
...
function record(result, item, warning) {
    result.receipt.push(item);
    if (warning) {
        result.warnings.push(warning);
    }
}
function skipLookup(result, kind, name, found, label) {
...
function unionNumbers(left, right) {
 ...
 */
async function adoptFirewall(existing, desired) {
    ...
    const rulesSame = firewallRulesEqual(existing.inbound_rules, inbound) &&
...
function domainRecordIdentity(entry) {
    return `${entry.type.trim().toUpperCase()} ${entry.name.trim()} ${entry.data.trim()}`;
}
async function ensureDomainRecord(domain, entry, liveRecords, result) {
    const identity = domainRecordIdentity(entry);
    const label = `${entry.type} ${entry.name}`;
    const receiptName = `${domain} ${label}`;
    const sameName = liveRecords.filter((item) => item.type?.trim().toUpperCase() === entry.type.trim().toUpperCase() && item.name?.trim() === entry.name.trim());
    ...
    if (exact.length > 1) {
        record(result, { kind: "domain_record", name: receiptName, action: "skipped", note: "ambiguous" }, `Skipping domain record ${identity} on "${domain}": ${exact.length} live records already match`);
        return "skipped";
    }
    if (sameName.length > 0) {
        record(result, { kind: "domain_record", name: receiptName, action: "skipped", note: "data differs" }, `Skipping domain record ${label} on "${domain}": a record with that type and name exists with different data and is not updated`);
        return "skipped";
    }
