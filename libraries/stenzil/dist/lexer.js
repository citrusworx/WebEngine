...
function isBracketStart(source, pos) {
...
function findBracketClose(source, innerStart) {
    let pos = innerStart;
...
function findCurlyClose(source, innerStart) {
    const close = source.indexOf("}}", innerStart);
    return close;
}
function classifyCurlyToken(value) {
    if (/^#.*#$/.test(value)) {
        return "Comment";
    }
...
function classifyBracketToken(value) {
    if (/^#.*#$/.test(value)) {
        return "Comment";
    }
...
function tokenize(source) {
    ...
    function advanceTo(nextPos) {
        while (pos < nextPos) {
            advanceChar(source[pos]);
            pos++;
        }
    }
    function flushText(until) {
        if (until <= textStart) {
            return;
        }
        tokens.push({
    ...
    while (pos < source.length) {
        if (source.startsWith("{{", pos)) {
            ...
            if (close === -1) {
                throw new types_1.LexerError("Unclosed curly directive", tokenLine, tokenCol);
            }
            flushText(pos);
        ...
        if (source[pos] === "[" && isBracketStart(source, pos)) {
            ...
            if (close === -1) {
                throw new types_1.LexerError("Unclosed bracket directive", tokenLine, tokenCol);
            }
            flushText(pos);
