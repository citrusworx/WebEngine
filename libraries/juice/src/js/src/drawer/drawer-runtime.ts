...
const OVERLAY_ID_PREFIX = 'juice-drawer-overlay';
...
export const createDrawer = (options: DrawerOptions = {}): DrawerController => {
  ...
  const ensureDialogAccessibility = (overlay: HTMLElement) => {
    ...
    if (
      ...
      const heading = dialog.querySelector<HTMLElement>(
        '[drawer-header] :is(h1,h2,h3,h4,h5,h6), :is(h1,h2,h3,h4,h5,h6)'
      );
      if (heading) {
        if (!heading.id) {
  ...
  const allowsBackdropClose = (overlay: HTMLElement) => {
    ...
    return overlay.getAttribute('drawer-overlay') !== 'static';
  };
...
export const initDrawer = (options: DrawerOptions = {}): DrawerController =>
  createDrawer(options);

let autoDrawerController: DrawerController | null = null;
...
export const startDrawerRuntime = (): DrawerController | null => {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    return null;
  }
  ...
  if (autoDrawerController) {
    autoDrawerController.sync();
    return autoDrawerController;
  }

  autoDrawerController = createDrawer();
  return autoDrawerController;
};

export const stopDrawerRuntime = () => {
  autoStartSuspended = true;
  cancelDeferredAutoStart();
  autoDrawerController?.destroy();
  autoDrawerController = null;
};
