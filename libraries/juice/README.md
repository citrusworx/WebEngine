...
- checkbox chrome colors come from `--juice-checkbox-*` roles bound by the active theme. `[checkbox]` is a binary APG checkbox on a button host (a native `<input type="checkbox" checkbox>` is allowed). Checked paint is `aria-checked="true"` / `:checked`. It is not `[switch]`, not menuitemcheckbox, and not tri-state. Toggle runtime is later.
- radio chrome colors come from `--juice-radio-*` roles bound by the active theme. `[radio]` is an APG radio on a button host inside `[radiogroup]` (a native `<input type="radio" radio>` is allowed). `[radiogroup]` is layout only. Checked paint is `aria-checked="true"` / `:checked`. Exclusive keyboard is later.
