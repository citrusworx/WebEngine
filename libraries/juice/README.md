...
- progress chrome colors come from `--juice-progress-*` roles bound by the active theme. `[progress]` is an APG-inspired progressbar track on `<div progress>`, with `[progress-fill]` for the completed portion and optional `[progress-label]`. Determinate width follows integer `aria-valuenow` 0–100 (other ranges use `--juice-progress-ratio`). `progress="indeterminate"` slides the fill. It is not a slider, not a spinner, and not a native `<progress>` restyle. Authors do not style the HTML `<progress>` element alone. Inline status chrome — no overlay. Theme chrome only — aria value sync and the indeterminate runtime are later. No Sig Progress factory.
