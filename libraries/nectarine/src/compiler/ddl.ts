...
export const DDL_VENDORS = ["postgres", "mysql"] as const;
...
type ColumnType =
    ...
    | { kind: "sized"; name: string; args: string[] }
...
function resolveVendor(vendor: string): DdlVendor {
    if (vendor === "mongodb") {
        throw new SchemaCompileError(
            "Schema DDL compilation supports postgres and mysql; MongoDB is not SQL CREATE TABLE",
        );
    }
...
function loadSchemaDoc(schema: unknown): Record<string, unknown> {
    if (typeof schema === "string") {
        const parsed = parser.yaml(schema);
        if (!isRecord(parsed)) {
...
function parseColumnType(spec: string): { type: ColumnType; rest: string } {
...
function takeDefaultValue(input: string): { value: DefaultValue; rest: string } {
...
function parseConstraintTokens(rest: string): Omit<FieldSpec, "name" | "type"> {
    let input = rest.trim();
    const result: Omit<FieldSpec, "name" | "type"> = {
        primaryKey: false,
        autoIncrement: false,
        unique: false,
        notNull: false,
    ...
    while (input.length > 0) {
        ...
        if (upper.startsWith("REFERENCES")) {
            const match = input.match(
                /^REFERENCES\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)/i,
            );
...
function parseStringField(name: string, spec: string): FieldSpec {
    const { type, rest } = parseColumnType(spec);
    return { name, type, ...parseConstraintTokens(rest) };
}

function parseDefaultFromObject(value: unknown): DefaultValue | undefined {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
function parseObjectField(name: string, spec: Record<string, unknown>): FieldSpec {
    ...
    if (typeof foreignKey === "string") {
        const refs = parseConstraintTokens(
            foreignKey.trim().toUpperCase().startsWith("REFERENCES")
                ? foreignKey
...
function parseField(name: string, spec: unknown): FieldSpec {
...
function parseIndexEntry(name: string, spec: unknown): IndexSpec {
...
function parseIndexes(table: string, indexes: unknown): IndexSpec[] {
...
function collectModels(schema: Record<string, unknown>): ModelSpec[] {
...
function namedSqlType(name: string, vendor: DdlVendor): string {
    switch (name) {
        case "string":
...
function emitSqlType(field: FieldSpec, vendor: DdlVendor): string {
    const incrementInt =
        field.autoIncrement &&
...
function sortModels(models: ModelSpec[]): ModelSpec[] {
    const byTable = new Map<string, ModelSpec>();
...
function planModels(models: ModelSpec[], vendor: DdlVendor): CompiledTable[] {
        ...
        references: [
            ...new Set(
                model.fields
                    ...
                    .filter((table): table is string => Boolean(table) && table !== model.table),
            ),
        ],
    }));
}
