...
exports.MAX_FRAGMENT_LENGTH = void 0;
exports.parseWhereFragment = parseWhereFragment;
exports.parseOrderByFragment = parseOrderByFragment;
exports.compiledValueToYaml = compiledValueToYaml;
exports.whereNodeToYaml = whereNodeToYaml;
exports.orderByToYaml = orderByToYaml;
const errors_js_1 = require("./errors.js");
const placeholders_js_1 = require("./placeholders.js");
exports.MAX_FRAGMENT_LENGTH = 1024;
const IDENTIFIER = /^[A-Za-z_][A-Za-z0-9_]*$/;
const KEYWORDS = new Set(["AND", "OR", "IN", "NOT", "IS", "NULL", "TRUE", "FALSE", "ASC", "DESC"]);
const TWO_CHAR_OPS = ["<=", ">=", "!=", "<>", "@>"];
function assertFragmentLength(input, label) {
    if (input.length > exports.MAX_FRAGMENT_LENGTH) {
        throw new errors_js_1.QueryCompileError(`${label} fragment exceeds ${exports.MAX_FRAGMENT_LENGTH} characters`);
    }
}
function tokenize(input, label) {
    ...
    while (i < input.length) {
        ...
        if (ch === "-" && input[i + 1] === "-") {
            throw new errors_js_1.QueryCompileError(`${label} fragment cannot contain comments`);
        }
        if (ch === "/" && input[i + 1] === "*") {
            throw new errors_js_1.QueryCompileError(`${label} fragment cannot contain comments`);
        }
        if (ch === ";") {
            throw new errors_js_1.QueryCompileError(`${label} fragment cannot contain semicolons`);
        }
        if (ch === "`" || ch === "\"") {
            throw new errors_js_1.QueryCompileError(`${label} fragment cannot contain quoted identifiers`);
        }
        if (ch === "$") {
            const match = input.slice(i).match(placeholders_js_1.PLACEHOLDER_SCAN);
            if (!match) {
                throw new errors_js_1.QueryCompileError(`${label} fragment has an invalid placeholder at ${i}`);
            }
        ...
        if (ch === "'") {
            ...
            if (!closed) {
                throw new errors_js_1.QueryCompileError(`${label} fragment has an unterminated string at ${i}`);
            }
            continue;
        }
        if (ch === "-" || /[0-9]/.test(ch)) {
            ...
            if (!match || (ch === "-" && !/[0-9]/.test(input[i + 1] ?? ""))) {
                throw new errors_js_1.QueryCompileError(`${label} fragment has an invalid token at ${i}`);
            }
        ...
        if (/[A-Za-z_]/.test(ch)) {
            ...
            if (!match) {
                throw new errors_js_1.QueryCompileError(`${label} fragment has an invalid identifier at ${i}`);
            }
            const word = match[0];
        ...
        throw new errors_js_1.QueryCompileError(`${label} fragment has an invalid character at ${i}`);
    }
    return tokens;
}
class FragmentParser {
    constructor(tokens, label) {
        this.index = 0;
        this.tokens = tokens;
        this.label = label;
    }
    parseWhere() {
        if (this.tokens.length === 0) {
            throw new errors_js_1.QueryCompileError("where fragment is empty");
        }
    ...
    parseOrderBy() {
        if (this.tokens.length === 0) {
            throw new errors_js_1.QueryCompileError("orderBy fragment is empty");
        }
        const terms = [];
    ...
    parsePredicate() {
        ...
        if (next?.kind !== "op") {
            throw new errors_js_1.QueryCompileError(`Invalid where fragment: expected operator after ${column}`);
        }
        this.index += 1;
        return {
            column,
    ...
    parseJsonPath() {
        ...
        while (this.peek()?.kind === "op" && (this.peek()?.value === "->" || this.peek()?.value === "->>")) {
            ...
            if (token?.kind !== "string") {
                throw new errors_js_1.QueryCompileError("JSONB path keys must be single-quoted strings");
            }
            this.index += 1;
    ...
    parseValue() {
        const token = this.peek();
        if (!token) {
            throw new errors_js_1.QueryCompileError("Invalid where fragment: expected a value");
        }
        ...
        throw new errors_js_1.QueryCompileError("Invalid where fragment: values must be $N, a boolean, a number, NULL, or a single-quoted string");
    }
    expectIdent(label) {
        const token = this.peek();
        if (token?.kind !== "ident" || !IDENTIFIER.test(token.value)) {
            throw new errors_js_1.QueryCompileError(`Invalid ${label} in ${this.label} fragment`);
        }
        this.index += 1;
        return token.value;
    }
    expectKeyword(word) {
        if (!this.peekKeyword(word)) {
            throw new errors_js_1.QueryCompileError(`Invalid where fragment: expected ${word}`);
        }
        this.index += 1;
    }
    expectPunct(value) {
        ...
        if (token?.kind !== "punct" || token.value !== value) {
            throw new errors_js_1.QueryCompileError(`Invalid where fragment: expected ${value}`);
        }
        this.index += 1;
    }
    peekKeyword(word) {
    ...
    expectEnd() {
        ...
        if (extra) {
            throw new errors_js_1.QueryCompileError(`Invalid ${this.label} fragment: unexpected token ${extra.value}`);
        }
    }
}
function operatorToken(op) {
    switch (op) {
        ...
        case "?":
            return "has_key";
        default:
            throw new errors_js_1.QueryCompileError(`Unknown operator: ${op}`);
    }
}
function parseWhereFragment(input) {
    if (typeof input !== "string") {
        throw new errors_js_1.QueryCompileError("where fragment must be a string");
    }
    return new FragmentParser(tokenize(input, "where"), "where").parseWhere();
}
function parseOrderByFragment(input) {
    if (typeof input !== "string") {
        throw new errors_js_1.QueryCompileError("orderBy fragment must be a string");
    }
    return new FragmentParser(tokenize(input, "orderBy"), "orderBy").parseOrderBy();
}
function compiledValueToYaml(value) {
...
function orderByToYaml(terms) {
    return terms.map((term) => term.direction ? { column: term.column, direction: term.direction } : { column: term.column });
}
