...
export type LessonContent = {
  ...
  heroImage: string;
  summary: string;
  tabs: Array<{ key: string; label: string; body: string[] }>;
  outline: Array<{ number: string; title: string; active?: boolean; complete?: boolean }>;
  instructor: { initials: string; name: string; role: string };
};

const SEED_POST: BlogPostContent = {
...
const SEED_LESSON: LessonContent = {
...
export async function getBlogPost(slug = "germanium-vs-silicon-fuzz"): Promise<BlogPostContent> {
  try {
    return await api.get<BlogPostContent>(`/api/posts/${encodeURIComponent(slug)}`);
  } catch {
    return { ...SEED_POST, slug };
  }
}

export async function getLesson(id = "bias-germanium-fuzz"): Promise<LessonContent> {
  try {
    return await api.get<LessonContent>(`/api/lessons/${encodeURIComponent(id)}`);
  } catch {
    return { ...SEED_LESSON, id };
  }
}
