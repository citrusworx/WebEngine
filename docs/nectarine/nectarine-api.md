...
Nectarine does **not** export `generateRoutes` and does not spin up a server. WebEngine / Blackwater hosts with **Seltzer**. Flatten `*API.yml` with Nectarine `listApiOperations`; Seltzer `generateRoutes` maps those operations onto `Route`s (copying `body` field specs onto `Route.contract`). Engine helpers `createNectarineReadRoutes` / `createNectarineWriteRoutes` / `createNectarineRoutes` call that path. Product JSONB catalog writes keep a host `execute` that runs `insertPayload` / `updatePayload` / `deleteProduct` (body is the catalog document, not flattened columns). Waitlist `joinWaitlist` also keeps a host `execute`; health and KiwiPress content stay hand-registered. Default Seltzer `validate` enforces `.required` keys; Nectarine can later `replace("validate", …)` for Zod.
