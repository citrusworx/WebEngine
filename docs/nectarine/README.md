...
4. Registers object-based Seltzer routes (resource **reads and YAML writes**, including jsonb-cast document POST/PUT/DELETE, plus waitlist POST `joinWaitlist` from `*API.yml` via `generateRoutes`)
