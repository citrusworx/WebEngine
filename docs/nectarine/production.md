...
Blackwater (`apps/blackwatersound/back`) loads config, connects, migrates, seeds, then `Seltzer.init().listen()`. Product **read** routes are generated from `productAPI.yml` via Seltzer `listApiOperations` + `generateRoutes` (handlers return `ResponseData`). Health, waitlist, and KiwiPress content stay hand-written. Leftover `ctx.json` / writing helpers crash or 500.
