...
- Seed inserts use named `seedPayload` (`ON CONFLICT (id) DO NOTHING`); HTTP create/update/delete use named JSONB queries (`insertPayload`, `updatePayload`, `deleteProduct`). Engine compiled execute now serializes the catalog document and skips `{ fn: now }`. Blackwater still passes a host `execute` for merge-on-PUT / 409 / memory fallback; it can drop that later. The compiler does not emit JSONB `||`.
