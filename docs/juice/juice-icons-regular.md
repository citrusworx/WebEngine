...
`regular.scss` defines the FontAwesome Regular icon set using attribute selectors. Each entry maps an `icon` attribute value to a regular-style SVG asset from the FontAwesome Free library.

> These icons are sourced from the FontAwesome Regular collection. Learn more at [FontAwesome Regular](https://fontawesome.com/icons?d=gallery&s=regular).
...
1. Add the SVG file to `libraries/juice/src/icons/fontawesome/web/svgs/regular/`.
2. Add a matching `[icon="..."]` rule to `libraries/juice/src/styles/icons/fontawesome/regular.scss`.

## Key details

- `regular.scss` is the file for FontAwesome Free Regular icons only.
- It is designed to work with the same `icon` attribute pattern as `brands.scss` and `solid.scss`.
