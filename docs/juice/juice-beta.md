...
- **Surfaces:** `surfaceTone="soft|strong|muted"`, `borderStrength="soft|bold"`, standalone `blur="sm|md"`, and `shadowTone="cool|warm"` — [juice-surfaces.md](./juice-surfaces.md). Tones, strength, and shadow tone use `--juice-surface-*` / `--juice-border-strength-*` / `--juice-shadow-tone-*` theme roles; blur uses fixed lengths (`6px` / `16px`).
