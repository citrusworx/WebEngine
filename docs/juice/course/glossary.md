...
**Surface tone.** Shipped paint contract: `surfaceTone="soft|strong|muted"` bound to `--juice-surface-*` roles. Composes with `borderStrength` (width) and standalone `blur="sm|md"` (frost length).
