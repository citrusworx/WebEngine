...
`stack` and `gap` are Juice. `theme="citrusmint"` is identity. Neither is a Vue/React component. Importing `@citrusworx/juiceui` (the JS entry) also auto-starts navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, slider, checkbox, and radio runtimes for valid markup. Switch, slider, checkbox, and radio are on master and are not in the 0.8.0 npm cut. Checkbox and radio are two runtimes. You do not need that for this lesson.
