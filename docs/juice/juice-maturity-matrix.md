...
This document tracks the current maturity of Juice across its major system areas. It reflects the **0.8.0** public cut on npm (wizard, tooltip, combobox, banner, eleven-runtime polish, and menu A→B→C). Master is ahead with the unpublished switch and slider runtimes (theme chrome, APG runtime, and runtime / maturity docs). Menu is the twelfth Emerging auto-enhance runtime in 0.8.0. Switch is the thirteenth Emerging auto-enhance runtime on master. Slider is the fourteenth Emerging auto-enhance runtime on master. Maturity ratings below stay **Emerging** — this pass does not bump anything to Stable-ish. 0.7.0 was the prior public cut.
...
| JS entrypoint | Emerging | Importing `@citrusworx/juiceui` auto-starts navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, and slider runtimes in the browser. Switch and slider are on master and are not in the 0.8.0 npm cut. Escape / layering is documented in [juice-runtime-behavior.md](./juice-runtime-behavior.md#escape--layering). Useful and documented, but still likely to evolve. |
| Public component exports | Emerging | Ships the Sig `Accordion` factory plus create/init/start/stop helpers for navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, and slider. Markup plus auto-enhance is the contract, not a large JS component library. There is no Sig Modal, Sig Drawer, Sig Toast, Sig Popover, Sig Wizard, Sig Tooltip, Sig Combobox, Sig Banner, Sig Menu, Sig Switch, or Sig Slider factory. |
...
- forms
- navigation
- accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, and slider auto-enhance runtimes
...
The JS entrypoint is no longer a stub. Navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, and slider auto-enhance on import, and `Accordion()` is a real Sig factory. Switch and slider are unpublished versus 0.8.0. That layer is Emerging, not the center of the Juice promise.
...
That framing matches the strongest current reality. The JS entry is an Emerging extra: importing it auto-enhances nav, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, and slider. It is not a finished component runtime.
...
- use the shipped auto-enhance runtimes (navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, slider) and the Sig Accordion factory as Emerging — valid markup should work after importing the JS entry, but that surface is not Stable-ish. Switch and slider are on master and are not in the 0.8.0 npm cut
