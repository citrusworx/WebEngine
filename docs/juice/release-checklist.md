...
- confirm interactive runtime tests still pass for navigation, accordion, tabs, and modal behavior
...
- `sideEffects` must include `./dist/index.js` as well as the CSS entries. The JS entry auto-starts navigation, accordion, tabs, and modal; bundlers that trust `sideEffects` can drop that auto-enhance if the JS file is treated as pure.
