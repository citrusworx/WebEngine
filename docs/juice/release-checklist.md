...
- confirm interactive runtime tests still pass for navigation, accordion, tabs, modal, drawer, toast, popover, and wizard behavior
...
- `sideEffects` must include `./dist/index.js` as well as the CSS entries. The JS entry auto-starts navigation, accordion, tabs, modal, drawer, toast, popover, and wizard; bundlers that trust `sideEffects` can drop that auto-enhance if the JS file is treated as pure.
