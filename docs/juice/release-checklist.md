...
- confirm interactive runtime tests still pass for navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, and switch behavior
...
- `sideEffects` must include `./dist/index.js` as well as the CSS entries. The JS entry auto-starts navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, and switch; bundlers that trust `sideEffects` can drop that auto-enhance if the JS file is treated as pure.
