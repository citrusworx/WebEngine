...
Indeterminate is `progress="indeterminate"` only. That flag wins over a stale `aria-valuenow` for paint. CSS slides `[progress-fill]`. The host clips that motion. `prefers-reduced-motion` stops the animation and leaves a static partial fill. The host is usually not focusable; `focus-ring` paints only on `:focus-visible`. Core CSS treats this as **inline** status chrome — no floating overlay z-index band. There is no progress runtime and no Sig Progress factory in this slice. Runtime B syncs `aria-valuenow` / min / max and the indeterminate flag (APG omits `aria-valuenow` while indeterminate).
