...
Structural APG-inspired pagination paint. Required names:

| Role | Job |
|---|---|
| `--juice-pagination-surface` | idle page-control fill |
| `--juice-pagination-surface-hover` | hover fill |
| `--juice-pagination-surface-current` | current page fill (`aria-current="page"`) |
| `--juice-pagination-ink` | idle control text |
| `--juice-pagination-ink-hover` | hover text |
| `--juice-pagination-ink-current` | text on the current page |
| `--juice-pagination-ink-disabled` | disabled prev/next text |
| `--juice-pagination-border` | control hairline |
...
A boolean `[pagination]` attribute is fine: there is no HTML global `pagination` attribute. Current-page paint is `aria-current="page"` on the control or its item. Disabled prev/next paint is `aria-disabled="true"`, native `disabled`, or `[disabled]`.

Core CSS treats this as **inline** nav chrome — no floating overlay z-index band. Color transitions honor `prefers-reduced-motion`. The pagination runtime auto-enhances that markup (`sync` / `setCurrent`): an unlabeled navigation landmark is named `Pagination`, `<nav>` does not get a redundant role, `<ol pagination>` stays a list, one `aria-current="page"` is kept (author current wins; otherwise the first page control), and prev/next disable at ends when that is inferable. Author-owned disabled is kept. Arrow / Home / End move focus among enabled controls. Enter and Space stay with the browser. It does not remove `href`, trap focus, or steal Escape. No Sig Pagination factory. Pagination is the nineteenth Emerging auto-enhance runtime and is unpublished versus 0.9.0. See [Pagination Runtime](./juice-pagination-runtime.md).

Tide must stay dark chips (`--tide-surface-strong`), not a light bar and not `--tide-page`. Current fill stays `--tide-accent`. Ink on that fill stays `--tide-heading`, not the page color.
