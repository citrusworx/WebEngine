...
This is Priority 2 through remaining depth slice C plus modal A→B→C, drawer A→B→C, toast A→B→C, and popover A→B→C: the checklist plus automated bind tests, including `shadowTone`, `overlay`, `variant` recipes, `--juice-modal-*`, `--juice-drawer-*`, `--juice-toast-*`, and `--juice-popover-*`. Blush, CLI, and publish are out of scope here.
