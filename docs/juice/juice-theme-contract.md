...
A boolean `[pagination]` attribute is fine: there is no HTML global `pagination` attribute. Current-page paint is `aria-current="page"` on the control or its item. Disabled prev/next paint is `aria-disabled="true"`, native `disabled`, or `[disabled]`. This slice does not invent ARIA and does not sync those flags.

Core CSS treats this as **inline** nav chrome — no floating overlay z-index band. Color transitions honor `prefers-reduced-motion`. There is no pagination runtime and no Sig Pagination factory in this slice. Runtime B can sync the current page and disabled prev/next. Eighteen Emerging runtimes stay the count until docs C.
...
This is Priority 2 through remaining depth slice C plus modal A→B→C, drawer A→B→C, toast A→B→C, popover A→B→C, wizard A→B→C, tooltip A→B→C, combobox A→B→C, banner A→B→C, menu A→B→C, switch A→B→C, slider A→B→C, checkbox A→B→C, radio A→B→C, breadcrumb A→B→C, and progress A→B→C (theme chrome, runtime, and runtime docs), plus pagination slice A (theme chrome only; runtime B — current page and disabled prev/next — and docs C are next): the checklist plus automated bind tests, including `shadowTone`, `overlay`, `variant` recipes, `--juice-modal-*`, `--juice-drawer-*`, `--juice-toast-*`, `--juice-banner-*`, `--juice-popover-*`, `--juice-tooltip-*`, `--juice-combobox-*`, `--juice-menu-*`, `--juice-switch-*`, `--juice-slider-*`, `--juice-checkbox-*`, `--juice-radio-*`, `--juice-breadcrumb-*`, `--juice-progress-*`, `--juice-pagination-*`, and `--juice-wizard-*`. Checkbox and radio are two runtimes. Breadcrumb is a light trail. Progress is a progressbar. Pagination chrome is unpublished versus 0.9.0. Eighteen Emerging runtimes stay the count. Blush, CLI, and publish are out of scope here.
