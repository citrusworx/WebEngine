...
**Master is ahead of 0.7.0.** Wizard A→B→C, tooltip A→B→C, and combobox A→B→C (theme chrome, runtime, runtime / maturity docs) live on master and are not in the published tarball. Do not run `yarn version-packages` or publish from this picture. Do not invent a next version number; the next cut happens when new Juice changesets are consumed.
...
* Emerging browser runtimes: navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, and combobox auto-enhance when the JS entry is imported
* a Sig Accordion factory plus create/init/start/stop helpers (no Sig Modal, Sig Drawer, Sig Toast, Sig Popover, Sig Wizard, Sig Tooltip, or Sig Combobox factory)
...
* accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, and combobox chrome plus DOM-first auto-enhance
...
* component maturity beyond the ten auto-enhance runtimes
...
* **Banner A→B.** Shared `--juice-banner-*` roles for `[banner]` / `[banner-body]` / `[banner-close]` (#160). DOM-first dismiss runtime: `createBanner` / `initBanner` / `startBannerRuntime` / `stopBannerRuntime`, auto-boot, `show` / `dismiss`, `[banner-close]` click / keyboard, `role="status"` (or `role="alert"` for error/warning), optional `banner-persist="session|local"` with `name` / `id`. No focus trap, no Escape steal, no auto-dismiss timer. Distinct from toast stack and from modal/drawer. No Sig Banner factory. Full runtime docs (slice C) later. Still unpublished vs 0.7.0.
...
Accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, and combobox have chrome plus auto-enhance. Navigation still exists and is still Emerging. The Sig Accordion factory is real. There is no Sig Modal, Sig Drawer, Sig Toast, Sig Popover, Sig Wizard, Sig Tooltip, or Sig Combobox factory.
...
These were the lock order after 0.4.0. Surfaces, the theme contract, and typography / icon polish shipped in the 0.6.0 public cut. Drawer / toast / popover shipped in 0.7.0. Wizard A→B→C, tooltip A→B→C, and combobox A→B→C are done on master (unpublished vs 0.7.0).
...
Combobox **A→B→C is done on master** (still unpublished vs 0.7.0): theme chrome (`--juice-combobox-*`), listbox runtime, and runtime / maturity docs. Valid `[combobox]` markup auto-enhances. Combobox is the tenth Emerging auto-enhance runtime. Do not oversell a component roadmap. A Sig Modal, Sig Drawer, Sig Toast, Sig Popover, Sig Wizard, Sig Tooltip, or Sig Combobox factory stays later. Grow the next runtime only when that markup contract stays honest.

Short-term focus remains:

* keep navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, and combobox documented as Emerging until they settle
...
Master is ahead of that cut. Wizard A→B→C, tooltip A→B→C, and combobox A→B→C are on master and unpublished.
...
* modal / dialog A→B→C is in 0.6.0; drawer A→B→C, toast A→B→C, and popover A→B→C are in 0.7.0; wizard A→B→C, tooltip A→B→C, and combobox A→B→C are done on master (still unpublished vs 0.7.0); grow the next runtime only when the markup contract is honest
