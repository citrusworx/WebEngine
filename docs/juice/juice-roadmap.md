...
* **Checkbox + radio A.** Shared `--juice-checkbox-*` and `--juice-radio-*` roles. `[checkbox]` is a binary APG checkbox (button host, native `<input type="checkbox" checkbox>` allowed). `[radio]` is an APG radio inside layout-only `[radiogroup]` (no `--juice-radiogroup-*`). Checked paint is `aria-checked="true"` / `:checked`. No tri-state, no runtime, no Sig factory. Next: B (checkbox toggle + radiogroup exclusive keyboard), then C (runtime docs / maturity). Still unpublished vs 0.8.0.
