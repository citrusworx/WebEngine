...
* Emerging browser runtimes: navigation, accordion, tabs, modal, drawer, toast, popover, wizard, tooltip, combobox, banner, menu, switch, slider, checkbox, radio, breadcrumb, and progress auto-enhance when the JS entry is imported. Switch, slider, checkbox, radio, breadcrumb, and progress are on master and are not in the 0.8.0 npm cut. Checkbox and radio are two runtimes. Breadcrumb is a light trail. Progress is a progressbar
...
Master is ahead of that cut. Switch A→B→C, slider A→B→C, checkbox A→B→C, radio A→B→C, breadcrumb A→B→C, and progress A→B→C are on master and unpublished. Switch is the thirteenth Emerging auto-enhance runtime. Slider is the fourteenth. Checkbox is the fifteenth. Radio is the sixteenth. Breadcrumb is the seventeenth. Progress is the eighteenth. Checkbox and radio are two runtimes. Breadcrumb is a light trail. Progress is a progressbar. Do not invent a next version number.
