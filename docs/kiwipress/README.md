...
`WPCreate` / `WPUpdate` / `WPDelete` exist as named wrappers around `mutate`. Posts, Pages, and Users extend `WPRead` and delegate `create()` to a `WPCreate` collaborator, `update()` to a `WPUpdate` collaborator, and `delete()` to a `WPDelete` collaborator.
