...
| `libraries/sig` | `@citrusworx/sigjs` | 0.2.0 standalone | Signals, effects, batch, memo, JSX-to-DOM, client router; no runtime deps | [Sig.js](./sigjs/README.md) · [package README](../libraries/sig/README.md) |
...
| `libraries/nectarine` | `@citrusworx/nectarine` | Active alpha | Compiler plus PostgreSQL, MySQL, and MongoDB adapters; WebEngine/Blackwater hosts HTTP with Seltzer | [Nectarine](./nectarine/README.md) |
...
| `engines/webengine` | `@citrusworx/webengine` | Early scaffold + working kiwi kernel | `WebEngine` lifecycle class, `kiwi.config.toml` loader, builtin modules (`core`/`web`/`native`/`embedded`/`nectarine`), Nectarine→Seltzer read/write routes, opt-in `startSeltzerFromKernel`, vitest coverage | [WebEngine](./webengine/README.md) · [Dual-process FE + API](./webengine/dual-process.md) |
