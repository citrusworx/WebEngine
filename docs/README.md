...
| `engines/webengine` | `@citrusworx/webengine` | Early scaffold + working kiwi kernel | `WebEngine` lifecycle class, `kiwi.config.toml` loader, builtin modules (`core`/`web`/`native`/`embedded`/`nectarine`), Nectarine→Seltzer read/write routes, vitest coverage | [WebEngine](./webengine/README.md) |
